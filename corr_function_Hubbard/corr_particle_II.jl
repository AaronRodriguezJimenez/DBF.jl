...
function run()
    ...
    heatmap(
        ordered_conn,
        xlabel = "site j (layout order)",
        ylabel = "site i (layout order)",
        title = "⟨C†_i C_j⟩ (ordered by layout=$layout)",
    ...
    ref_site = clamp(div(Nsites+1,2), 1, Nsites)  # center-ish
    println("Reference site chosen: ", ref_site)
    ...
    # --------- Plot 3: correlation vs Manhattan distance (binned average) ----------
    # Need (row,col) position for each site in the chosen layout.
    function site_pos_in_layout(site_num::Int, Lx::Int, Ly::Int, site_order::Vector{Int})
        # find index in site_order (position in grid), then compute row/col
        pos = findfirst(==(site_num), site_order)
        row = fld(pos-1, Lx) + 1
        col = ((pos-1) % Lx) + 1
        return (row, col)
    end

    # positions for all sites
    pos_list = [site_pos_in_layout(s, Lx, Ly, site_order) for s in 1:Nsites]
    ref_pos = site_pos_in_layout(ref_site, Lx, Ly, site_order)

    # compute Manhattan distances and group
    dists = Int[]
    for s in 1:Nsites
        r,c = pos_list[s]
        rr,cc = ref_pos
        push!(dists, abs(r-rr) + abs(c-cc))
    end
    maxdist = maximum(dists)

     # accumulate correlator by distance
    sum_by_d = zeros(maxdist+1)
    count_by_d = zeros(Int, maxdist+1)
    for s in 1:Nsites
        d = dists[s]
        sum_by_d[d+1] += site_conn[ref_site, s]   # d+1 because distances start at 0
        count_by_d[d+1] += 1
    end

    avg_by_d = sum_by_d ./ max.(1, count_by_d)
    # plot averaged correlation vs distance
    distances = collect(0:maxdist)
    plot(distances, avg_by_d,
        seriestype = :line,
        marker = :circle,
        xlabel = "Manhattan distance from site $ref_site",
        ylabel = "Avg connected correlation",
     #   title = "Distance-binned connected correlation",
        label = "avg ⟨n($ref_site) n(r)⟩_c",
    )
    savefig("corr_vs_distance_ref$(ref_site).png")
    println("Saved corr_vs_distance_ref$(ref_site).png")
end
