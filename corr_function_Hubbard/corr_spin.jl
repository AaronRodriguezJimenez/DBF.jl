...
function get_spin_correlation_function(ψ::Ket{N}, occ, i, j, g, θ, ϵcoeff, ϵweight) where N
...
function get_spin_correlation_function(ψ::Ket{N}, occ,i, g, θ, ϵcoeff, ϵweight) where N
    S = PauliSum(N, ComplexF64)
    S += Pauli(N, X=[i])
    S += Pauli(N, Y=[i])
    S += Pauli(N, Z=[i])
...
function run()
    ...
    cs = zeros(N) 
    cse = zeros(N) 
    vs, vse, S = get_spin_correlation_function(ψ, occ, i, g, θ, thresh, max_weight)
    ...
    sites = collect(2:N)
    for j in sites 
        vs, vse, S = get_spin_correlation_function(ψ, occ, j, g, θ, thresh, max_weight)
    ...
    for j in sites 
        ...
        vs, vse, S = get_spin_correlation_function(ψ,occ, i, j, g, θ, thresh, max_weight)
...
savefig("spin_correlation_hubbard.png")
